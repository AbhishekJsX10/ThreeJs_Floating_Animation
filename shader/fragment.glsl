...
void main() {
    ...
    float strength = smoothstep(0.0, 1.0, uTimeline) - smoothstep(4.0, 5.0, uTimeline);
    ...
    float changeTimeline = smoothstep(1.0, 6.5, uTimeline);
    float waveMask = smoothstep(0.4, 0.6, wave + uTimeline * 0.1);

    vec4 tex = mix(startTexture, endTexture, waveMask);
